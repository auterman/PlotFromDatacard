...
int main(int argc, char* argv[])
{
   ...
   TH1F * a = new TH1F("template",";Signal region;Event yield",imax,+0.5,imax+0.5);
   ...
   leg->prepend(*s,"Signal, GGM M_{1} = M_{2} = 1 TeV","l");
   ...
   d->Draw("pe0,same");
   tot->Draw("h,same");
   TLegend * dleg = new TLegend(leg->buildLegend(0.47,0.68,0.9,0.9));
   dleg->SetNColumns(2);
   dleg->Draw("same");
   ...
   atext->DrawLatex(1.5,1.1*syst->GetMaximum(),"#gamma+ST");
   atext->DrawLatex(5.0,1.1*syst->GetMaximum(),"#gamma+HT");
   atext->DrawLatex(13.5,1.1*syst->GetMaximum(),"#gamma+lepton");
   ...
   TLegend * sleg = new TLegend(0.7,0.89,0.9,0.999);


   TH1F hRatio=hist::getRatio(*d,*tot,"Data / Prediction ",hist::ONLY1);
   TGraphErrors grRatioStat=hist::getRatioGraph(*stat,*tot,"Ratio",hist::ONLY1);
   TGraphErrors grRatioSyst=hist::getRatioGraph(*syst,*tot,"Ratio",hist::ONLY1);
   hRatio.GetXaxis()->SetTitle("Signal region");
   ...
   gfx::scaleXerrors(grRatioStat,.99);
   gfx::scaleXerrors(grRatioSyst,.85);
   ...
   aline->DrawLine(0,1,49,1);  
   sleg->AddEntry(&grRatioStat,"Stat.","f");
